...
`frontend/src/components/A2UI/catalogs/<pipeline>/catalog.json` — **50 trusted components** on the default pipeline `prompt-composer` as of 2026-09-19 (`ecommerce` carries 38): 17 A2UI Basic Catalog primitives (`Column`, `Row`, `Text`, `Card`, `Button`, `Image`, `ActionGroup`, `SectionEditor`, `DecisionDialog`, `ConsoleCardGrid`, `CompiledOutput`, `ChatPanel`, `TraceFeed`, `AgentFlow`, `AgentCanvas`, `OutputControls`, `CanvasFooter`) plus 33 workspace-specific Lit elements (`workspace-layout`, `prompt-section-editor`, `compiled-output-viewer`, `chat-panel`, `trace-feed`, `agent-flow`, `agent-canvas`, `output-controls`, `canvas-footer`, `chat-repair-actions`, `version-trace`, `token-cost-readout`, `user-response-bubble`, …).

**The count and the names are asserted on every run — by the checker, not by the startup log.** `catalog-check.mjs` (`doc-claim-drift`, blocking) reads the number and the enumerated list out of [`README.md`](../README.md) §Component Catalog and this section, and holds them against the catalog, so drift between catalog and docs fails the build instead of waiting to be remembered. The startup line (`✅ A2UI Catalog loaded — 50 trusted components`, `deps.py`) prints the live number and compares it to nothing: it was previously described here as an assertion, and it never was one — which is how this count sat at 28 while the catalog grew to 37.
