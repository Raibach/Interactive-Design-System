...
- **The division of labour.** `backend/governance_inspector.py` runs the catalog check, reads `open-items.json` and `corrections.json`, and reads git. For each of the six rows it produces the evidence and, where the answer is arithmetic, the answer itself. No model reads the repository.
- **Two models cross-check.** The auditor classifies the six rows into two values. A second model receives the same sheet with the auditor's answers and reports agreement or names the rows that the evidence does not support. The run reports `ok` when the reviewer agrees, `attention` when it disagrees, and `unverified` when no reviewer answered. Each disagreement is stored with both answers.
- **The roster assigns models by measurement.** `inspection.json` lists candidate models for each role. A model loads for the run and unloads when the run ends, so its memory returns to the machine. The deep role tries candidates in order and records every attempt. Measured: the 9B reasoning model returned no message at 700 and at 2500 output tokens, and the `/no_think` directive reduced its answer time from 19s to 4s.
- **What the inspection checks.** The register's recorded counts against the counts this run derives; blocking findings against their register rows; open rows against the presence of a witness; the `error-suppression` count against its recorded value; uncommitted files in the tree; and the change since the previous run. Every row carries the evidence that the checker or the code produced.
- **A silent failure becomes a row.** The checker counts swallowed failures on the governed path, which are empty catch blocks, exception blocks that only print, and promise handlers that answer with a default. A new site changes the count; the register must match; the build fails when the two disagree.
- **Reporting reaches the console and the trace.** The verdict is written as a message in the console's own conversation, which is the one global conversation in the system. Package conversations remain bound to their packages. The seat writes each inspection message to the application logger on load, and the Trace tab displays it.
- **A failed run is recorded, not replaced.** The protocol in `inspection.json` defines the questions, the two-value vocabulary, the roster, and the output shape. An answer outside that shape, or a run where a model does not answer, is stored as NOT DONE with the raw text. No run reaches for a different provider.
- **Schedule, cost, and the model selector.** The backend runs one inspection each day from its lifespan, and `POST /api/governance/inspect` starts a run on demand. A completed run used 1.2s and 3.1s for the two classifiers, 0.3s for the reviewer, and 1.5GB of model memory during the run. The models in the local library will also populate the package model selector, so a prompt and an agentic flow can run on each of them in turn.

**[2026-09-18] — Known, not fixed: the grippers move by travel, not by the cursor**

Written down at the owner's request so it is not mistaken for working, and to be deleted from this file when it is fixed. A gripper sizes its pane by how far the pointer has travelled since the press (`workspace-layout.ts`, `_onMouseMove`: `startWidth + (e.clientX - _startX)`), not by where the pointer is. So when the layout reflows mid-drag, or the pane reaches its minimum and stops, the pointer and the edge come apart and stay apart for the rest of the gesture — the divider should sit under the cursor. The chat input's height gripper is the same defect from the other side: `chat-action-bar.ts` raises `input-resize-start` / `-move` / `-end`, and `workspace-layout.ts` wires **start and move to the same start handler**. Recorded in [`TO-DO.md`](TO-DO.md) with what "fixed" means.

**[2026-09-17] — One model call, no second attempt: the assembly path stops hiding its own failures**

The console load fired three model calls where it needed one, and every one of them raced the others against a 10-second contract the code was quietly working around. Measured before: **23 of 86** `assemble-surface` requests returned 503 (27%), the model milestone ranged **3.6–20.8s**, and the database — which people assume is the slow part — measured **6–65ms**. Measured after: one call, **2.70 / 2.72 / 2.78 / 2.85 / 2.96s**, no failures, and cards on screen on a fresh page load.

- **Three calls, two of them identical.** The mount effect assembled the console *and* Grace's catalog report in the same millisecond (`Promise.allSettled`), and the report request went out **twice** — a `<StrictMode>` remount re-ran the effect down a path that deliberately bypassed the single in-flight slot. All three hit one provider at once, so each pushed the others past the cap, and the one that had to draw the screen was the one refused. The load is in sequence now: the surface assembles, alone; the checker's findings are read **after it lands**, from the audit report the checker already wrote (`GET /api/catalog/audit`) — a file read, not a model call. Measured on a fresh page: **one** assembly request, 2.84s, 200, cards drawn, zero alerts, the findings read 1–2ms after the surface returned.
- **The retry was the suppression.** `max_retries=1` against a 10s-per-attempt client meant a 19.6s "success" was attempt-1-timeout plus attempt-2-answer: the contract the surface is held to was never the contract that ran, and a slow answer was reported as a fast one. There was also a provider loop named, in the code itself, a **"fallback loop"**. Both are gone — one provider, **one attempt**, `max_retries=0`. A failure is final at 10 seconds, and the only way to run it again is a person pressing Retry.
- **The same class one layer down:** an empty answer used to be replaced by the model's `reasoning_content`, so the output column could show an inner monologue dressed as a result. It now raises, naming the finish reason.
- **A surface does not reason.** The model was spending 460 completion tokens thinking about a job whose answer its own prompt dictates, character for character — guessing where there was nothing to guess. Told not to (`reasoning_effort: "none"`, surfaces only): **460 tokens / 6.05s → 251 / 1.53s** on the same prompt, the same valid JSON. Real console assemblies went from 3.4–20.8s at a 27% failure rate to ~2.8s with none.
- **Compute, per console load:** three calls at ~3,760 tokens each (**~11,280**) became one at **3,551** — about **68% fewer tokens** and roughly ten times less wall time. Each eliminated 503 also stops wasting two full attempts (~7,500 tokens) on a screen that never appeared.
- **Temperature and reasoning have one home now, chosen by mode** in `grace_gui.py`. It was three homes and none of them was the intended number: the chat element sent `0.45`, the request model defaulted to `0.45`, and the backend passed through whatever arrived. Surfaces pin **0.0** (it used to be pinned for `console_assembly` alone, so every other surface inherited its caller's number); `chat` is the one conversation, at **`CHAT_TEMPERATURE = 1.5`**, reasoning allowed; **`writer`** is the new *default* mode — tags, summaries, source evaluation, memory recall — at 0.0 with no reasoning. The default is the load-bearing part: `query_llm`'s default was `"chat"`, so five tag-extraction call sites and four helpers would have silently inherited reasoning the moment the rule was applied. A caller that forgets its mode now gets the executor's behaviour, not the conversationalist's. Verified live on `/api/memory/recall`: 200, one provider call, `one attempt, 120s`.
- **"It was new, so nothing had to check it" is closed.** A new check, `element-refused` (**blocking**), refuses a **behavioural** element — one that dispatches an event — that no allowlist entry, catalog entry, Figma-map entry or drawing parent claims. Claiming recognises all four homes now, including a parent element composing a sub-piece (seven chat sub-pieces were reported as unclaimed for the lack of it), which took `element-unclaimed` **7 → 0** honestly rather than by exemption. The `TODO(behavior)` escape is gone as well: a comment written inside a file could excuse the event *that file dispatches* — a claim the code made about itself.
- **The register's numbers are true again.** `event-unheard` 17 → **18**, the number `OPEN-ITEMS.md` had recorded, so `open-items-register` passes; open findings **45 → 38**; blocking **2 → 0**. The last one was the missing `CORRECTIONS.md` (`#021`) — restored from `retired-files/`, every row it carries still earned — and the register's recorded count for `corrections-ledger` corrected to the 0 this run derives. The gate that ends `npm run build`, and therefore the deploy, now passes.
- **`ANNOTATION_REVIEW_WORKFLOW.md` no longer pre-classifies by example.** "The spacer" was listed under **decor** — "visual only, neither content nor interactive" — while its own master carries `On drag: dispatch input-resize-start/move/end`. Read from the file, not the code: the set `chat-left-spacer` (40001085:2598) carries nothing; its variant `state=Default` (40001085:2597) carries the drag contract. Kinds now come from evidence — the annotation, then the source — and the page says so.
- **Not seen, stated rather than implied:** the chat's 1.5-with-reasoning is verified only as far as the provider accepting those settings (1.5 with reasoning 3.02s; the no-reasoning setting 2.15s at fewer tokens) — no chat turn was sent, because it would write a test message into a real conversation. `reasoning` and `reasoning_style` are still accepted by three callers and **read by nobody**. The composer's whole tree, its chat column included, is still built and never displayed: 61 shadow roots on one console page, with two of nearly every element.
...
The console chat is **global**. It operates on cards, never on packages — it reads no package's conversations and writes none. Its whole job is the console experience: reshuffle and filter which cards show, and later plugins, metrics and console customization. That is the full list.

- **Ownership.** Every conversation must carry a `session_id` (`NOT NULL`, FK → `prompt_sessions`). The console chat has none today, so it persists nothing. It gets **one console-typed `prompt_sessions` row per user**, created when the user lands on the console — the console is the entry point, so every user passes through it.
- **Get-or-create, not create-if-missing.** Two tabs racing on load would otherwise make two console sessions and split the chat. The database enforces one console session per user, rather than the code checking first.
- **Per-tab conversations are already modelled.** `conversations.tab` exists, indexed on `(session_id, tab, updated_at DESC)`. No schema change needed.
- **The reshuffle mechanism already exists.** The assistant emits `<reassemble-console sort="…" filter="…"/>`, handled as an `a2ui:console-command`. Plugins, metrics and customization later are more commands in that same channel.

**Consequence for the build:** the console's chat becomes the same Lit `<chat-panel>` in the surface that assembles the cards, and the React seat in the operator shell retires. The panel it becomes is described in the entry below.

**[2026-09-17] — Chat panel: the design is the source of truth**

The composer's chat panel was brought into line with the Figma design, and one old path removed. A summary:

- **The rail is four buttons** — Chat, Trace, Versions, Tools. Evaluate and Metadata came off: they were placeholders whose own panels said "design pending". `Variables` was renamed `Versions`, because version history is what that panel actually renders.
- **Every rail button is a component with named states** — `Default`, `Hover`, `Selected`, plus `Alert` (fades to blank, 400 ms, holding a static tint under reduced-motion) and `Unknown` (static, for "the check could not run"). Notes sit on the variants, one per behaviour state.
- **Send and stop became one control.** The design has a single `send-stop-chat-input` with two states; the bar was drawing two buttons side by side.
- **The Conversations dropdown carries the conversation list** and dispatches `conversation-select`, as its annotation says. The duplicate selector that lived inside the thread is gone.
