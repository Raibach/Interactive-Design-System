...
| 3 | She is scoped to a package, not global | **VIOLATED** | `frontend/src/shared/chatSeat.ts:45` holds the conversation in a module-level singleton; `clearChatSeat()` has no caller outside tests; no history load on package switch |
