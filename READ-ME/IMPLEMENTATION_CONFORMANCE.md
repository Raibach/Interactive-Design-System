...
| **R4** Allowlist enforcement before client delivery | `validate_a2ui_components()` runs at all 4 envelope return sites; unknown type or missing `id` → HTTP 503 with §1 error; catalog loads at startup **fail-fast** (`sys.exit(1)` if unreadable) from `catalogs/<pipeline>/catalog.json` — currently **55 trusted components** on the default pipeline (`ecommerce` carries 38), printed live at boot. `npm run catalog:check` compares the docs' count against the catalog (`doc-claim-drift`, blocking, run on request — **not** part of `npm run build`). **What this validator does NOT check: props.** It reads `component` and `id` and nothing else, so a prop the catalog does not declare, and a `children` shape the catalog contradicts, are both accepted at runtime — measured 2026-09-23 | ✅ |
...
| **R6** Declarative-only, zero executable code | No executable code anywhere: `eval()` has zero hits in `frontend/src`; button handlers dispatch declarative `a2ui:action` CustomEvents; the tags a model reply can name are validated against the registry before dispatch (`frontend/src/shared/event-bus.ts` — `validateTag`), so the `<set-html>` a reply may still carry is parsed and REJECTED there, with no listener to serve it. (The retired `A2UISurfaceContainer.tsx` this row used to cite — that file is not in `src` — is at `retired-files/dead-code-audit-20260916/`.) | ✅ |
...
`frontend/src/components/A2UI/catalogs/<pipeline>/catalog.json` — **55 trusted components** on the default pipeline `prompt-composer` as of 2026-09-23 (`ecommerce` carries 38): 17 A2UI Basic Catalog primitives (`Column`, `Row`, `Text`, `Card`, `Button`, `Image`, `ActionGroup`, `SectionEditor`, `DecisionDialog`, `ConsoleCardGrid`, `CompiledOutput`, `ChatPanel`, `TraceFeed`, `AgentFlow`, `AgentCanvas`, `OutputControls`, `CanvasFooter`) plus 38 workspace-specific Lit elements (`workspace-layout`, `prompt-section-editor`, `compiled-output-viewer`, `chat-panel`, `chat-header`, `trace-feed`, `agent-flow`, `agent-canvas`, `output-controls`, `canvas-footer`, `chat-repair-actions`, `version-trace`, `token-cost-readout`, `user-response-bubble`, …).

**The count is asserted when the checker is run; the NAMES are not.** `npm run catalog:check` (`doc-claim-drift`, blocking) reads the number out of this section and [`README.md`](../README.md) §Component Catalog — the regex is `(\d+) trusted components` per line — and holds it against the catalog. **It is run on request and is NOT part of `npm run build`:** the build is `tsc -b && vite build`, and an earlier version of this line claimed the build enforced it, which it did not. **What it does NOT read is the enumerated list**: the names above are prose, no checker parses them, and a name dropped from the list while the count stayed right would pass. The startup line (`✅ A2UI Catalog loaded — 55 trusted components`, `deps.py`) prints the live number and compares it to nothing: it was previously described here as an assertion, and it never was one — which is how this count sat at 28 while the catalog grew to 37, and again at 51 while the catalog grew to 55.
